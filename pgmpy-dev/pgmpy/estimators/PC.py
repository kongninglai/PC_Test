...
class PC(StructureEstimator):
    ...
    #             results = Parallel(backend= 'loky', n_jobs=n_jobs)(
    ...
    @staticmethod
    def _parallel_fun(u, v, graph, ci_test, lim_neighbors, data, independencies, significance_level, **kwargs):
        for separating_set in chain(
            combinations(set(graph.neighbors(u)) - set([v]), lim_neighbors),
            combinations(set(graph.neighbors(v)) - set([u]), lim_neighbors),
        ):
        ...
        while not all(
            ...
            if variant == "orig":
                ...
                results = Parallel(n_jobs=n_jobs, backend='loky')(
                    delayed(PC._parallel_fun)(u, v, graph, ci_test, lim_neighbors, self.data, self.independencies, significance_level, **kwargs)
                    for (u, v) in graph.edges()
                )
                for result in results:
                    if result is not None:
